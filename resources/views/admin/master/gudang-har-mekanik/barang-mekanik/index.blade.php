@extends('layouts.app')
@section('title', __("Data Mechanical PLTGU Tanjung Uncang"))
@section('content')
<x-head-datatable/>
            ...
            <div class="card w-100">
                <div class="card-header row">
                    <div class="d-flex justify-content-between align-items-center w-100">
                        <!-- Bagian Kiri (Template dan Import Data) -->
                        <div class="d-flex">
                            @if(Auth::user()->role->name != 'staff')
                                <a href="{{ route('barang-mekanik.template') }}" class="btn btn-outline-success btn-sm font-weight-bold m-1">
                                    <i class="fas fa-download m-1"></i>Template
                                </a>
                    
                                <button class="btn btn-success btn-sm font-weight-bold m-1" type="button" data-toggle="modal" data-target="#importModal">
                                    <i class="fas fa-upload m-1"></i>{{ __("Import Data") }}
                                </button>
                            @endif
                        </div>
                    
                        <!-- Bagian Kanan (Export dan Add Data) -->
                        <div class="d-flex">
                    
                            @if(Auth::user()->role->name != 'staff')
                                <button class="btn btn-success btn-sm font-weight-bold m-1" type="button" data-toggle="modal" data-target="#TambahData" id="modal-button">
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    <div class="col-md-7">
                                        ...
                                        <div class="form-group">
                                            <label for="date_items_mechanical" class="form-label">{{ __("Keterangan") }} <span class="text-danger">*</span></label>
                                            <input type="date" name="date_items_mechanical" class="form-control">
                                        </div>
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <div id="pdfPreviewContainer" style="margin-top: 15px;">
...
<div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('barang-mekanik.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="file">{{ __("Choose Excel File") }}</label>
                        <input type="file" name="file" class="form-control" required>
...
<script>
    ...
    function previewPDF(event) {
        ...
        if (pdfFile.type === "application/pdf") {
            ...
            pdfPreview.style.display = 'block';  // Show the object container
            pdfPreview.data = URL.createObjectURL(pdfFile);  // Set the PDF file to display
            pdfPreview.onload = function() {
                URL.revokeObjectURL(pdfPreview.data); // Free up memory after loading
            }
        } else {
    ...
    $('#data-tabel').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: `{{ route('barang-mekanik.list') }}`,
        columns: [
            {
            ...
            {
                data: 'date_items_mechanical',
                name: 'date_items_mechanical',
                render: function (data) {
                    if (data) {
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    if (data) {
                        let pdfIconUrl = "{{ asset('images/pdf-icon.png') }}"; // Lokasi icon PDF
                        let pdfUrl = "{{ asset('storage/pdf_files_mekanik/') }}/" + data; // Lokasi file PDF
                        return "<a href='#' class='pdf-link' data-pdf-url='" + pdfUrl + "'><img src='" + pdfIconUrl + "' style='width:20px; height:20px; margin-right:5px;' alt='PDF Icon'/> Show PDF</a>";
                    }
                    return 'no files'; // Kosong jika tidak ada file PDF
                },
                orderable: false,
                searchable: false
            },
    ...
    function simpan() {
        ...
        Form.append('date_items_mechanical', $("input[name='date_items_mechanical']").val());
        Form.append('person_name', $("input[name='person_name']").val());
        Form.append('file_reference', $("input[name='file_reference']")[0].files[0]);
        Form.append('unit_name', $("input[name='unit_name']").val());
        Form.append('brand_name', $("input[name='brand_name']").val());

    $.ajax({
        url: `{{ route('barang-mekanik.save') }}`,
        type: "POST",
        data: Form,
        processData: false,
        contentType: false,
...
function resetForm() {
    ...
    $("input[name='date_items_mechanical']").val('');
...
$(document).ready(function() {
    ...
    $("#modal-button").on("click", function() {
        ...
        $("input[name='date_items_mechanical']").val(null);
    ...
    swalWithBootstrapButtons.fire({
        title: "Anda Yakin ?",
        text: "Data Pada Transaksi Masuk dan Keluar Akan Di Hapus",
        icon: "warning",
