@extends('layouts.app')
@section('title',__('Transaksi Keluar Mekanik'))
@section('content')
<x-head-datatable/>
            ...
            <div class="card w-100">
                <div class="card-header row">
                    ...
                    <div class="d-flex justify-content-end align-items-center w-100">
                        <button class="btn {{$in_status_mechanical!=0?'btn-success':'btn-danger'}}" type="button"  data-toggle="modal" {{$in_status_mechanical!=0?'data-target="#TambahData"':'data-target="alert"'}} id="modal-button"><i class="fas fa-plus m-1"></i> {{__('add data')}}</button>
                    </div>
                    @endif
                </div>
...
<script>
    ...
    function simpan(){
        ...
        $.ajax({
            url:`{{route('transaksi.keluar.mekanik.save')}}`,
            type:"post",
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    // Buat objek FormData untuk mengirimkan data
    ...
    $.ajax({
        url: `{{route('transaksi.keluar.mekanik.update')}}`,
        type: "POST",
        data: formData,
    ...
    $(document).ready(function(){
        $('#data-tabel').DataTable({
            ...
            processing:true,
            serverSide:true,
            ajax:`{{route('transaksi.keluar.mekanik.list')}}`,
            columns:[
                {
        ...
        $("#modal-button").on("click",function(){
            ...
            id = new Date().getTime();
            $("input[name='kode']").val("BRGKLRMEKANIK-"+id);
            $("input[name='id']").val(null);
            $("input[name='id_barang']").val(null);
            // $("select[name='customer']").val('-- Pilih Customer --');
            $("input[name='part_name']").val(null);
    ...
    $(document).on("click",".ubah",function(){
        ...
        let id = $(this).attr('id');
        $.ajax({
            url:"{{route('transaksi.keluar.mekanik.detail')}}",
            type:"post",
            data:{
                id:id,
            },
    ...
    $(document).on("click",".hapus",function(){
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{route('transaksi.keluar.mekanik.delete')}}",
                    type:"delete",
                    data:{
                        id:id
                    },
