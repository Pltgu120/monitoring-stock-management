@extends('layouts.app')
@section('title',__('Transaksi Masuk Kimia'))
@section('content')
<x-head-datatable/>
...
<script>
    ...
    function load(){
        $('#data-barang').DataTable({
            ...
            processing:true,
            serverSide:true,
            ajax:`{{route('barang.transaksi.masuk.kimia.list.in')}}`,
            columns:[
                {
...
<script>
    function detail(){
        ...
        $.ajax({
            url:`{{route('barang-kimia.part_number')}}`,
            type:'post',
            data:{
    ...
    function simpan(){
        const item_chemical_id =  $("input[name='id_barang']").val();
        const user_id = `{{Auth::user()->id}}`;
        const date_received_chemical = $("input[name='tanggal_masuk']").val();
        ...
        Form.append('item_chemical_id',item_chemical_id);
        Form.append('date_received_chemical', date_received_chemical );
        Form.append('quantity', quantity );
        Form.append('invoice_number', invoice_number );
        Form.append('image', image );
        $.ajax({
            url:`{{route('transaksi.masuk.kimia.save')}}`,
            type:"post",
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    formData.append('item_chemical_id', item_chemical_id);
    formData.append('user_id', user_id);
    formData.append('date_received_chemical', date_received_chemical);
    ...
    // Jika metode PUT diperlukan
    formData.append('_method', 'POST');

    $.ajax({
        url: `{{route('transaksi.masuk.kimia.update')}}`,
        type: "POST", // Tetap gunakan POST jika Laravel mendukung `_method`
        data: formData,
        processData: false, // Mencegah jQuery memproses data
        contentType: false, // Membiarkan browser menetapkan header yang sesuai
        success: function(res) {
            Swal.fire({
                position: "center",
                icon: "success",
    ...
    $(document).ready(function(){
        $('#data-tabel').DataTable({
            ...
            processing:true,
            serverSide:true,
            ajax:`{{route('transaksi.masuk.kimia.list')}}`,
            columns:[
                {
                    "data":null,"sortable":false,
                    render:function(data,type,row,meta){
                ...
                {
                    data: "date_received_chemical",
                    name: "date_received_chemical",
        ...
        $("#modal-button").on("click",function(){
            ...
            id = new Date().getTime();
            $("input[name='kode']").val("BRGMSKKIMIA-"+id);
            $("input[name='id']").val(null);
            $("input[name='id_barang']").val(null);
            $("input[name='part_name']").val(null);
            $("input[name='tanggal_masuk']").val(null);
    ...
    $(document).on("click",".ubah",function(){
        ...
        let id = $(this).attr('id');
        $.ajax({
            url:"{{route('transaksi.masuk.kimia.detail')}}",
            type:"post",
            data:{
                id:id,
            },
            success:function({data}){
                ...
                $("input[name='part_name']").val(data.part_name);
                $("input[name='tanggal_masuk']").val(data.date_received_chemical);
                $("input[name='part_number']").val(data.part_number);
    ...
    $(document).on("click",".hapus",function(){
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{route('transaksi.masuk.kimia.delete')}}",
                    type:"delete",
                    data:{
                        id:id
                    },
