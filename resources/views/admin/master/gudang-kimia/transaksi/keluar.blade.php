@extends('layouts.app')
@section('title',__('Transaksi Keluar Kimia'))
@section('content')
<x-head-datatable/>
            ...
            <div class="card w-100">
                <div class="card-header row">
                    ...
                    <div class="d-flex justify-content-end align-items-center w-100">
                        <button class="btn {{$in_status_chemical!=0?'btn-success':'btn-danger'}}" type="button"  data-toggle="modal" {{$in_status_chemical!=0?'data-target="#TambahData"':'data-target="alert"'}} id="modal-button"><i class="fas fa-plus m-1"></i> {{__('add data')}}</button>
                    </div>
                    @endif
                </div>
...
<script>
    ...
    function simpan(){
        ...
        $.ajax({
            url:`{{route('transaksi.keluar.kimia.save')}}`,
            type:"post",
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    const invoice_number = $("input[name='kode']").val();
    const quantity = $("input[name='jumlah']").val();
    const image = $("input[name='image']")[0].files[0]; // Perbaikan pada penulisan selector
    ...
    if (image) { // Pastikan ada file sebelum menambahkannya
        formData.append("image", image);
    }

    $.ajax({
        url: `{{route('transaksi.keluar.kimia.update')}}`,
        type: "POST", // Gunakan POST untuk mendukung FormData
        data: formData,
    ...
    $(document).ready(function(){
        $('#data-tabel').DataTable({
            ...
            processing:true,
            serverSide:true,
            ajax:`{{route('transaksi.keluar.kimia.list')}}`,
            columns:[
                {
        ...
        $("#modal-button").on("click",function(){
            ...
            id = new Date().getTime();
            $("input[name='kode']").val("BRGKLRKIMIA-"+id);
            $("input[name='id']").val(null);
            $("input[name='id_barang']").val(null);
            // $("select[name='customer']").val('-- Pilih Customer --');
            $("input[name='part_name']").val(null);
    ...
    $(document).on("click",".ubah",function(){
        ...
        let id = $(this).attr('id');
        $.ajax({
            url:"{{route('transaksi.keluar.kimia.detail')}}",
            type:"post",
            data:{
                id:id,
            },
    ...
    $(document).on("click",".hapus",function(){
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{route('transaksi.keluar.kimia.delete')}}",
                    type:"delete",
                    data:{
                        id:id
                    },
