@extends('layouts.app')
@section('title', __("Edit Barang Bekas Pakai PLTGU Tanjung Uncang"))
@section('content')
<x-head-datatable/>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card w-100">
                ...
                <div class="card-body">
                    <form action="{{ route('barang-bekas-pakai.update', $consumable_item->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-7">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="part_name" class="form-control" value="{{ $consumable_item->part_name }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="part_number" class="form-label">{{ __("Part Number") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="part_number" class="form-control" value="{{ $consumable_item->part_number }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="kode_rak" class="form-label">{{ __("Kode Rak") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="kode_rak" class="form-control" value="{{ $consumable_item->kode_rak }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="date_consumable_items" class="form-label">{{ __("Keterangan") }} <span class="text-danger">*</span></label>
                                    <input type="date" name="date_items" class="form-control" value="{{ $consumable_item->date_consumable_items }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="person_name" class="form-label">{{ __("Person Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="person_name" class="form-control" value="{{ $consumable_item->person_name }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="unit_name" class="form-label">{{ __("Unit Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="unit_name" class="form-control" value="{{ $consumable_item->unit_name }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="brand_name" class="form-label">{{ __("Brand Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="brand_name" class="form-control" value="{{ $consumable_item->brand_name }}" required>
                                </div>

                                <div class="form-group">
                                    <label for="quantity" class="form-label">{{ __("Quantity") }} <span class="text-danger">*</span></label>
                                    <input type="number" name="quantity" class="form-control" value="{{ $consumable_item->quantity }}" min="0" required>
                                </div>
