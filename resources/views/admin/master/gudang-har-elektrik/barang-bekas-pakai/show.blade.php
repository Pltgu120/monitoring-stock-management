@extends('layouts.app')
@section('title', __("Detail Barang Bekas Pakai PLTGU Tanjung Uncang"))
@section('content')
<x-head-datatable/>
                        ...
                        <div class="row">
                            <div class="col-md-7">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="part_name" class="form-control" value="{{ $consumable_item->part_name }}" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="part_number" class="form-label">{{ __("Part Number") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="part_number" class="form-control" value="{{ $consumable_item->part_number }}" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="kode_rak" class="form-label">{{ __("Kode Rak") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="kode_rak" class="form-control" value="{{ $consumable_item->kode_rak }}" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="date_consumable_items" class="form-label">{{ __("Keterangan") }} <span class="text-danger">*</span></label>
                                    <input type="date" name="date_items" class="form-control" value="{{ $consumable_item->date_consumable_items }}" required readonly> 
                                </div>

                                <div class="form-group">
                                    <label for="person_name" class="form-label">{{ __("Person Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="person_name" class="form-control" value="{{ $consumable_item->person_name }}" required readonly>
                                </div>
                                

                                <div class="form-group">
                                    <label for="unit_name" class="form-label">{{ __("Unit Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="unit_name" class="form-control" value="{{ $consumable_item->unit_name }}" min="0" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="brand_name" class="form-label">{{ __("Brand Name") }} <span class="text-danger">*</span></label>
                                    <input type="text" name="brand_name" class="form-control" value="{{ $consumable_item->brand_name }}" min="0" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="initial_qty" class="form-label">{{ __("Initial Quantity") }} <span class="text-danger">*</span></label>
                                    <input type="number" name="initial_qty" class="form-control" value="{{ $consumable_item->initial_qty }}" min="0" required readonly>
                                </div>

                                <div class="form-group">
                                    <label for="quantity" class="form-label">{{ __("Quantity") }} <span class="text-danger">*</span></label>
                                    <input type="number" name="quantity" class="form-control" value="{{ $consumable_item->quantity }}" min="0" required readonly>
                                </div>
                            ...
                            <div class="col-md-5">
                                <div class="form-group">
                                    ...
                                    @if($consumable_item->image)
                                        <embed src="{{ asset('storage/barang_bekas_pakai/' . $consumable_item->image) }}" width="100%" height="300px" class="mb-2" />
                                    @endif
