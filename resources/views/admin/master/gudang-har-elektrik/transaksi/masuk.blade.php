@extends('layouts.app')
@section('title',__('Transaksi Masuk Sparepart'))
@section('content')
<x-head-datatable/>
...
<script>
    ...
    // Image preview when the user selects a file
...
// In case of form submission and saving the image, make sure to return the correct image URL from server-side
function ubah() {
    ...
    const invoice_number = $("input[name='kode']").val();
    const quantity = $("input[name='jumlah']").val();
    const image = $("input[name='image']")[0].files[0]; // Get the selected image file
    ...
    if (image) { // Append the file only if selected
        Form.append('image', image);
    }

    $.ajax({
        url: `{{route('transaksi.masuk.update')}}`,
        type: "POST", // Use POST if PUT isn't available
        processData: false, // Don't process the data, let FormData handle it
        contentType: false, // Let FormData handle the content type header
        data: Form,
        success: function(res) {
            Swal.fire({
                position: "center",
            ...
            $('#kembali').click(); // Close the modal
            resetForm(); // Reset the form inputs after success
            $('#data-tabel').DataTable().ajax.reload(); // Reload the data table

            // Update the image preview to the URL of the stored image
            const imageUrl = `{{ asset('storage/barang_masuk') }}/${res.image_path}`;
    ...
    $(document).ready(function(){
        ...
        $("#modal-button").on("click",function(){
            ...
            id = new Date().getTime();
            $("input[name='kode']").val("BRGMSKELEKTRIK-"+id);
